...
| `create_drafts` | Create up to 10 posts from Markdown or bounded native `heading`, `paragraph`, `callout`, and `button` blocks; always draft-only. |
| `create_page_drafts` | Create up to 10 Pages from the same Markdown or bounded native-block input; always draft-only. |
